...
describe("config cli", () => {
  ...
  describe("config set parsing flags", () => {
    ...
    it("shows --strict-json and keeps --json as a legacy alias in help", async () => {
      ...
      expect(helpText).toContain("superhuman config set gateway.port 19001 --strict-json");
      expect(helpText).toContain(
        "superhuman config set channels.discord.token --ref-provider default --ref-source",
      );
