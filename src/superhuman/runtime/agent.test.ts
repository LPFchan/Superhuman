...
import { SuperhumanAgentRuntimeTurn } from "./agent.js";
  ...
  it("derives verified outcomes from explicit verification tool results", () => {
    ...
    turn.handleAgentEvent({
      ...
      data: {
        ...
        name: "bash",
        toolCallId: "verify-1",
        args: { command: "pnpm test -- src/superhuman/runtime/agent.test.ts" },
      },
    });
    turn.handleAgentEvent({
      runId: "run-verified",
    ...
    expect(stateStore.getActions({ runId: "run-verified" })).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          command: "pnpm test -- src/superhuman/runtime/agent.test.ts",
          exitCode: 0,
        }),
      ]),
    );
