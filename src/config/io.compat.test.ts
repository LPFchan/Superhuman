...
async function writeConfig(
  home: string,
  dirname: ".superhuman" | ".openclaw",
  port: number,
  filename: string = "superhuman.json",
) {
...
describe("config io paths", () => {
  it("uses legacy ~/.openclaw/openclaw.json when it already exists", async () => {
    await withTempHome(async (home) => {
      const configPath = await writeConfig(home, ".openclaw", 19001, "openclaw.json");
  ...
  it("defaults to ~/.superhuman/superhuman.json when config is missing", async () => {
    await withTempHome(async (home) => {
      const io = createIoForHome(home);
      expect(io.configPath).toBe(path.join(home, ".superhuman", "superhuman.json"));
    });
  });

  it("uses SUPERHUMAN_HOME for default config path", async () => {
    await withTempHome(async (home) => {
      const io = createConfigIO({
        env: { SUPERHUMAN_HOME: path.join(home, "svc-home") } as NodeJS.ProcessEnv,
        homedir: () => path.join(home, "ignored-home"),
      });
      expect(io.configPath).toBe(path.join(home, "svc-home", ".superhuman", "superhuman.json"));
    });
  });

  it("honors explicit SUPERHUMAN_CONFIG_PATH override", async () => {
    await withTempHome(async (home) => {
      const customPath = await writeConfig(home, ".superhuman", 20002, "custom.json");
      const io = createIoForHome(home, { SUPERHUMAN_CONFIG_PATH: customPath } as NodeJS.ProcessEnv);
