---
summary: "CLI reference for `superhuman onboard` (interactive onboarding)"
read_when:
...
```bash
superhuman onboard
superhuman onboard --flow quickstart
superhuman onboard --flow manual
superhuman onboard --mode remote --remote-url wss://gateway-host:18789
```
...
```bash
superhuman onboard --non-interactive \
...
- `--install-daemon` starts the managed gateway install path first. Without it, you must already have a local gateway running, for example `superhuman gateway run`.
...
- Fastest first chat: `superhuman dashboard` (Control UI, no channel setup).
