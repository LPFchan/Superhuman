---
summary: "CLI reference for `superhuman secrets` (reload, audit, configure, apply)"
read_when:
...
Use `superhuman secrets` to manage SecretRefs and keep the active runtime snapshot healthy.
...
```bash
superhuman secrets audit --check
superhuman secrets configure
superhuman secrets apply --from /tmp/openclaw-secrets-plan.json --dry-run
superhuman secrets apply --from /tmp/openclaw-secrets-plan.json
superhuman secrets audit --check
...
```bash
superhuman secrets reload
superhuman secrets reload --json
```
...
Scan Superhuman state for:

- plaintext secret storage
- unresolved refs
- precedence drift (`auth-profiles.json` credentials shadowing `superhuman.json` refs)
...
```bash
superhuman secrets audit
superhuman secrets audit --check
superhuman secrets audit --json
superhuman secrets audit --allow-exec
```
...
```bash
superhuman secrets configure
superhuman secrets configure --plan-out /tmp/openclaw-secrets-plan.json
superhuman secrets configure --apply --yes
superhuman secrets configure --providers-only
superhuman secrets configure --skip-provider-setup
superhuman secrets configure --agent ops
superhuman secrets configure --json
```
...
- `configure` targets secret-bearing fields in `superhuman.json` plus `auth-profiles.json` for the selected agent scope.
...
- On Windows, if ACL verification is unavailable for a provider path, Superhuman fails closed. For trusted paths only, set `allowInsecurePath: true` on that provider to bypass path security checks.
...
```bash
superhuman secrets apply --from /tmp/openclaw-secrets-plan.json
superhuman secrets apply --from /tmp/openclaw-secrets-plan.json --allow-exec
superhuman secrets apply --from /tmp/openclaw-secrets-plan.json --dry-run
superhuman secrets apply --from /tmp/openclaw-secrets-plan.json --dry-run --allow-exec
superhuman secrets apply --from /tmp/openclaw-secrets-plan.json --json
```
...
- `superhuman.json` (SecretRef targets + provider upserts/deletes)
- `auth-profiles.json` (provider-target scrubbing)
- legacy `auth.json` residues
- `~/.superhuman/.env` known secret keys whose values were migrated
