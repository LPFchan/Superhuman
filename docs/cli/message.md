---
summary: "CLI reference for `superhuman message` (send + channel actions)"
read_when:
...
```
superhuman message <subcommand> [flags]
```
...
- On cache miss, Superhuman will attempt a live directory lookup when the provider supports it.
...
- `superhuman message` resolves supported channel SecretRefs before running the selected action.
- Resolution is scoped to the active action target when possible:
  ...
  - when `--account` is omitted, Superhuman does not force a `default` account SecretRef scope
...
```
superhuman message send --channel discord \
...
```
superhuman message poll --channel discord \
  --target channel:123 \
  --poll-question "Snack?" \
...
```
superhuman message poll --channel telegram \
  --target @mychat \
  --poll-question "Lunch?" \
  --poll-option Pizza --poll-option Sushi \
...
```
superhuman message poll --channel msteams \
...
```
superhuman message react --channel slack \
...
```
superhuman message react --channel signal \
...
```
superhuman message send --channel telegram --target @mychat --message "Choose:" \
...
```bash
superhuman message send --channel telegram --target @mychat \
