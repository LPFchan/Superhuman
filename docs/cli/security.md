---
summary: "CLI reference for `superhuman security` (audit and fix common security footguns)"
read_when:
...
```bash
superhuman security audit
superhuman security audit --deep
superhuman security audit --deep --password <password>
superhuman security audit --deep --token <token>
superhuman security audit --fix
superhuman security audit --json
```
...
It also emits `security.trust_model.multi_user_heuristic` when config suggests likely shared-user ingress (for example open DM/group policy, configured group targets, or wildcard sender rules), and reminds you that Superhuman is a personal-assistant trust model by default.
...
It also warns when existing sandbox browser Docker containers have missing/stale hash labels (for example pre-migration containers missing `openclaw.browserConfigEpoch`) and recommends `superhuman sandbox recreate --browser --all`.
...
```bash
superhuman security audit --json | jq '.summary'
superhuman security audit --deep --json | jq '.findings[] | select(.severity=="critical") | .checkId'
```

If `--fix` and `--json` are combined, output includes both fix actions and final report:

```bash
superhuman security audit --fix --json | jq '{fix: .fix.ok, summary: .report.summary}'
```
