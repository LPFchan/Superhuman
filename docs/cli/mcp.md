---
summary: "Expose Superhuman channel conversations over MCP and manage saved MCP server definitions"
read_when:
  - Connecting Codex, Claude Code, or another MCP client to Superhuman-backed channels
  - Running `superhuman mcp serve`
  - Managing Superhuman-saved MCP server definitions
title: "mcp"
---

# mcp

`superhuman mcp` has two jobs:

- run Superhuman as an MCP server with `superhuman mcp serve`
- manage Superhuman-owned outbound MCP server definitions with `list`, `show`,
  `set`, and `unset`

In other words:

- `serve` is Superhuman acting as an MCP server
- `list` / `show` / `set` / `unset` is Superhuman acting as an MCP client-side
  registry for other MCP servers its runtimes may consume later

Use [`superhuman acp`](/cli/acp) when Superhuman should host a coding harness
session itself and route that runtime through ACP.

## Superhuman as an MCP server

This is the `superhuman mcp serve` path.

## When to use `serve`

Use `superhuman mcp serve` when:

- Codex, Claude Code, or another MCP client should talk directly to
  Superhuman-backed channel conversations
- you already have a local or remote Superhuman Gateway with routed sessions
- you want one MCP server that works across Superhuman's channel backends instead
  of running separate per-channel bridges

Use [`superhuman acp`](/cli/acp) instead when Superhuman should host the coding
runtime itself and keep the agent session inside Superhuman.

## How it works

`superhuman mcp serve` starts a stdio MCP server. The MCP client owns that
process. While the client keeps the stdio session open, the bridge connects to a
local or remote Superhuman Gateway over WebSocket and exposes routed channel
conversations over MCP.

Lifecycle:

1. the MCP client spawns `superhuman mcp serve`
...
conversations. A conversation appears when Superhuman already has session state
...
```bash
# Local Gateway
superhuman mcp serve

# Remote Gateway
superhuman mcp serve --url wss://gateway-host:18789 --token-file ~/.superhuman/gateway.token

# Remote Gateway with password auth
superhuman mcp serve --url wss://gateway-host:18789 --password-file ~/.superhuman/gateway.password

# Enable verbose bridge logs
superhuman mcp serve --verbose

# Disable Claude-specific push notifications
superhuman mcp serve --claude-channel-mode off
```
...
Superhuman equivalent of a Claude Code channel adapter: standard MCP tools remain
...
- sender allowlists, pairing, and channel-level trust still belong to the
  underlying Superhuman channel configuration
...
Superhuman ships a deterministic Docker smoke for this bridge:
...
- starts a second container that spawns `superhuman mcp serve`
...
## Superhuman as an MCP client registry

This is the `superhuman mcp list`, `show`, `set`, and `unset` path.

These commands do not expose Superhuman over MCP. They manage Superhuman-owned MCP
server definitions under `mcp.servers` in Superhuman config.

Those saved definitions are for runtimes that Superhuman launches or configures
later, such as embedded Pi and other runtime adapters. Superhuman stores the
definitions centrally so those runtimes do not need to keep their own duplicate
MCP server lists.

Important behavior:

- these commands only read or write Superhuman config
...
Superhuman also stores a lightweight MCP server registry in config for surfaces
that want Superhuman-managed MCP definitions.

Commands:

- `superhuman mcp list`
- `superhuman mcp show [name]`
- `superhuman mcp set <name> <json>`
- `superhuman mcp unset <name>`

Examples:

```bash
superhuman mcp list
superhuman mcp show context7 --json
superhuman mcp set context7 '{"command":"uvx","args":["context7-mcp"]}'
superhuman mcp set docs '{"url":"https://mcp.example.com"}'
superhuman mcp unset context7
```
