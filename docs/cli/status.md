---
summary: "CLI reference for `superhuman status` (diagnostics, probes, usage snapshots)"
read_when:
...
```bash
superhuman status
superhuman status --all
superhuman status --deep
superhuman status --usage
```
...
- Update info surfaces in the Overview; if an update is available, status prints a hint to run `superhuman update` (see [Updating](/install/updating)).
