---
summary: "CLI reference for `superhuman daemon` (legacy alias for gateway service management)"
read_when:
  - You still use `superhuman daemon ...` in scripts
...
`superhuman daemon ...` maps to the same service control surface as `superhuman gateway ...` service commands.

## Usage

```bash
superhuman daemon status
superhuman daemon install
superhuman daemon start
superhuman daemon stop
superhuman daemon restart
superhuman daemon uninstall
```
...
Use [`superhuman gateway`](/cli/gateway) for current docs and examples.
