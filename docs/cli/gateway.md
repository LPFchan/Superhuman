---
summary: "Superhuman Gateway CLI (`superhuman gateway`) — run, query, and discover gateways"
read_when:
...
The Gateway is Superhuman’s WebSocket server (channels, nodes, sessions, hooks).

Subcommands in this page live under `superhuman gateway …`.
...
```bash
superhuman gateway run
```

Notes:

- By default, the Gateway refuses to start unless `gateway.mode=local` is set in `~/.superhuman/superhuman.json`. Use `--allow-unconfigured` for ad-hoc/dev runs.
...
```bash
superhuman gateway health --url ws://127.0.0.1:18789
```
...
superhuman gateway status --require-rpc
```
...
```bash
superhuman gateway probe
superhuman gateway probe --json
```
...
```bash
superhuman gateway probe --ssh user@gateway-host
```
...
```bash
superhuman gateway call status
superhuman gateway call logs.tail --params '{"sinceMs": 60000}'
```
...
superhuman gateway start
superhuman gateway stop
superhuman gateway restart
superhuman gateway uninstall
```
...
```bash
superhuman gateway discover --timeout 4000
superhuman gateway discover --json | jq '.beacons[].wsUrl'
```
