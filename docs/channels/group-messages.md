...
Add a `groupChat` block to `~/.superhuman/superhuman.json` so display-name pings work even when WhatsApp strips the visual `@` in the text body:
...
1. Add your WhatsApp account (the one running Superhuman) to the group.
...
- Session store entries will appear as `agent:<agentId>:whatsapp:group:<jid>` in the session store (`~/.superhuman/agents/<agentId>/sessions/sessions.json` by default); a missing entry just means the group hasn’t triggered a run yet.
