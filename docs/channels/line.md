...
read_when:
  - You want to connect Superhuman to LINE
...
LINE connects to Superhuman via the LINE Messaging API. The plugin runs as a webhook
...
```bash
superhuman plugins install @openclaw/line
```

Local checkout (when running from a git repo):

```bash
superhuman plugins install ./path/to/local/line-plugin
```
...
- LINE signature verification is body-dependent (HMAC over the raw body), so Superhuman applies strict pre-auth body limits and timeout before verification.
- Superhuman processes webhook events from the verified raw request bytes. Upstream middleware-transformed `req.body` values are ignored for signature-integrity safety.
...
```bash
superhuman pairing list line
superhuman pairing approve line <CODE>
```
