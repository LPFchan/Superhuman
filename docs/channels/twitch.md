...
read_when:
  - Setting up Twitch chat integration for Superhuman
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. Superhuman connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
superhuman plugins install @openclaw/twitch
```

Local checkout (when running from a git repo):

```bash
superhuman plugins install ./path/to/local/twitch-plugin
```
