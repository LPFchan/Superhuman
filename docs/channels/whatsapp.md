...
- Onboarding (`superhuman onboard`) and `superhuman channels add --channel whatsapp`
  prompt to install the WhatsApp plugin the first time you select it.
- `superhuman channels login --channel whatsapp` also offers the install flow when
...
```bash
superhuman plugins install @openclaw/whatsapp
```
...
```bash
superhuman channels login --channel whatsapp --account work
```
...
```bash
superhuman pairing list whatsapp
superhuman pairing approve whatsapp <CODE>
```
...
<Note>
Superhuman recommends running WhatsApp on a separate number when possible. (The channel metadata and setup flow are optimized for that setup, but personal-number setups are also supported.)
</Note>
...
<AccordionGroup>
  <Accordion title="Dedicated number (recommended)">
    ...
    - separate WhatsApp identity for Superhuman
  ...
  <Accordion title="WhatsApp Web-only channel scope">
    The messaging platform channel is WhatsApp Web-based (`Baileys`) in current Superhuman channel architecture.
...
<AccordionGroup>
  ...
  <Accordion title="Credential paths and legacy compatibility">
    - current auth path: `~/.superhuman/credentials/whatsapp/<accountId>/creds.json`
    - backup file: `creds.json.bak`
    - legacy default auth in `~/.openclaw/credentials/` is still recognized/migrated for default-account flows
  </Accordion>

  <Accordion title="Logout behavior">
    `superhuman channels logout --channel whatsapp [--account <id>]` clears WhatsApp auth state for that account.
...
<AccordionGroup>
  ...
  <Accordion title="Group messages unexpectedly ignored">
    ...
    - duplicate keys in `superhuman.json` (JSON5): later entries override earlier ones, so keep a single `groupPolicy` per scope
