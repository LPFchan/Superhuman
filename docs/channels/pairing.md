...
read_when:
  ...
  - Reviewing Superhuman security posture
title: "Pairing"
---

# Pairing

“Pairing” is Superhuman’s explicit **owner approval** step.
...
Stored under `~/.superhuman/credentials/`:
...
3. On your phone, open the Superhuman iOS app → Settings → Gateway.
...
Stored under `~/.superhuman/devices/`:
...
- The legacy `node.pair.*` API (CLI: `superhuman nodes pending/approve`) is a
