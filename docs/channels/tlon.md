...
Tlon is a decentralized messenger built on Urbit. Superhuman connects to your Urbit ship and can
...
```bash
superhuman plugins install @openclaw/tlon
```

Local checkout (when running from a git repo):

```bash
superhuman plugins install ./path/to/local/tlon-plugin
```
...
By default, Superhuman blocks private/internal hostnames and IP ranges for SSRF protection.
...
Use these with `superhuman message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Superhuman replies in-thread.
