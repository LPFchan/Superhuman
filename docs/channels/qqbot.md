...
read_when:
  - You want to connect Superhuman to QQ
...
QQ Bot connects to Superhuman via the official QQ Bot API (WebSocket gateway). The
...
## Bundled with Superhuman

Current Superhuman installs bundle QQ Bot. You do not need a separate
`superhuman plugins install` step for normal setup.
...
```bash
superhuman channels add --channel qqbot --token "AppID:AppSecret"
```
...
- `superhuman channels add --channel qqbot --token-file ...` provides the
...
Run multiple QQ bots under a single Superhuman instance:
...
```bash
superhuman channels add --channel qqbot --account bot2 --token "222222222:secret-of-bot-2"
```
...
| `/bot-version` | Show the Superhuman framework version |
