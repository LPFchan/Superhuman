...
read_when:
  - Setting up Zalo Personal for Superhuman
...
Status: experimental. This integration automates a **personal Zalo account** via native `zca-js` inside Superhuman.
...
- Install via CLI: `superhuman plugins install @openclaw/zalouser`
- Or from a source checkout: `superhuman plugins install ./path/to/local/zalouser-plugin`
...
2. Login (QR, on the Gateway machine):
   - `superhuman channels login --channel zalouser`
...
```bash
superhuman directory self --channel zalouser
superhuman directory peers list --channel zalouser --query "name"
superhuman directory groups list --channel zalouser --query "work"
```
...
- `superhuman pairing list zalouser`
- `superhuman pairing approve zalouser <code>`
...
- On startup, Superhuman resolves group/user names in allowlists to IDs and logs the mapping.
...
- When a group message is skipped because mention is required, Superhuman stores it as pending group history and includes it on the next processed group message.
...
Accounts map to `zalouser` profiles in Superhuman state. Example:
...
- Superhuman sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, Superhuman sends delivered + seen acknowledgements (best-effort).
...
- Re-login: `superhuman channels logout --channel zalouser && superhuman channels login --channel zalouser`
...
- The channel now runs fully in Superhuman without external CLI binaries.
