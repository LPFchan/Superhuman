...
```bash
superhuman plugins install @openclaw/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
superhuman plugins install ./path/to/local/nextcloud-talk-plugin
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "Superhuman" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `superhuman pairing list nextcloud-talk`
  - `superhuman pairing approve nextcloud-talk <CODE>`
