---
summary: "Synology Chat webhook setup and Superhuman config"
read_when:
  - Setting up Synology Chat with Superhuman
...
```bash
superhuman plugins install ./path/to/local/synology-chat-plugin
```
...
1. Install and enable the Synology Chat plugin.
   - `superhuman onboard` now shows Synology Chat in the same channel setup list as `superhuman channels add`.
   - Non-interactive setup: `superhuman channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
2. In Synology Chat integrations:
   - Create an incoming webhook and copy its URL.
   - Create an outgoing webhook with your secret token.
3. Point the outgoing webhook URL to your Superhuman gateway:
   - `https://gateway-host/webhook/synology` by default.
   - Or your custom `channels.synology-chat.webhookPath`.
4. Finish setup in Superhuman.
   - Guided: `superhuman onboard`
   - Direct: `superhuman channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
...
- Pairing approvals work with:
  - `superhuman pairing list synology-chat`
  - `superhuman pairing approve synology-chat <CODE>`
...
```bash
superhuman message send --channel synology-chat --target 123456 --text "Hello from Superhuman"
superhuman message send --channel synology-chat --target synology-chat:123456 --text "Hello again"
```
...
Give each enabled account a distinct `webhookPath`. Superhuman now rejects duplicate exact paths
