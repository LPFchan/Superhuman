...
```bash
superhuman plugins install @openclaw/msteams
```

Local checkout (when running from a git repo):

```bash
superhuman plugins install ./path/to/local/msteams-plugin
```

If you choose Teams during setup and a git checkout is detected,
Superhuman will offer the local install path automatically.
...
3. Configure Superhuman with those credentials.
...
- Talk to Superhuman via Teams DMs, group chats, or channels.
...
- On startup, Superhuman resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.superhuman/superhuman.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring Superhuman, you need to create an Azure Bot resource.
...
1. **Install the Microsoft Teams plugin**
   - From npm: `superhuman plugins install @openclaw/msteams`
   - From a local checkout: `superhuman plugins install ./path/to/local/msteams-plugin`
...
Superhuman exposes a Graph-backed `member-info` action for Microsoft Teams so agents and automations can resolve channel member details (display name, email, role) directly from Microsoft Graph.
...
```json5
{
  ...
  description: { short: "Superhuman in Teams", full: "Superhuman in Teams" },
...
Superhuman handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, Superhuman only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/SuperhumanShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

Superhuman sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `superhuman message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.superhuman/msteams-polls.json`.
...
```bash
superhuman message send --channel msteams \
...
superhuman message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
superhuman message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
superhuman message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
superhuman message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
