...
<Tabs>
  <Tab title="Socket Mode (default)">
    <Steps>
      ...
      <Step title="Configure Superhuman">
...
```
  ...
  <Tab title="HTTP Events API mode">
    <Steps>
      ...
      <Step title="Configure Superhuman HTTP mode">
...
<Tabs>
  <Tab title="DM policy">
    ...
    Pairing in DMs uses `superhuman pairing approve slack <code>`.
...
- The interactive callback values are Superhuman-generated opaque tokens, not raw agent-authored values.
- If generated interactive blocks would exceed Slack Block Kit limits, Superhuman falls back to the original text reply instead of sending an invalid blocks payload.
...
`typingReaction` adds a temporary reaction to the inbound Slack message while Superhuman is processing a reply, then removes it when the run finishes. This is a useful fallback when Slack native assistant typing is unavailable, especially in DMs.
...
```json
{
  "display_information": {
    "name": "Superhuman",
    "description": "Slack connector for Superhuman"
  },
  "features": {
    "bot_user": {
    ...
    "slash_commands": [
      {
        "command": "/openclaw",
        "description": "Send a message to Superhuman",
        "should_escape": false
      }
    ]
  },
...
Superhuman supports Slack native text streaming via the Agents and AI Apps API.
...
- If streaming fails mid-reply, Superhuman falls back to normal delivery for remaining payloads.
