...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects Superhuman to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.

---

## Bundled plugin

Feishu ships bundled with current Superhuman releases, so no separate plugin install
is required.
...
```bash
superhuman plugins install @openclaw/feishu
```
...
If you just installed Superhuman, run onboarding:
...
2. Configuring app credentials in Superhuman
...
1. You already ran `superhuman channels add` for Feishu
2. The gateway is running (`superhuman gateway status`)
...
Edit `~/.superhuman/superhuman.json`:
...
```bash
superhuman pairing approve feishu <CODE>
```
...
- **Approve pairing**:
  ...
  ```bash
  superhuman pairing list feishu
  superhuman pairing approve feishu <CODE>
  ```
...
2. Run `superhuman logs --follow` and look for `chat_id`
...
2. Run `superhuman logs --follow` and look for `open_id`
...
| Command                      | Description                   |
| ---------------------------- | ----------------------------- |
| `superhuman gateway status`  | Show gateway status           |
| `superhuman gateway install` | Install/start gateway service |
| `superhuman gateway stop`    | Stop gateway service          |
| `superhuman gateway restart` | Restart gateway service       |
| `superhuman logs --follow`   | Tail gateway logs             |
...
4. Check logs: `superhuman logs --follow`
...
5. Ensure the gateway is running: `superhuman gateway status`
6. Check logs: `superhuman logs --follow`
