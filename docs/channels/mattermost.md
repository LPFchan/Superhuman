---
summary: "Mattermost bot setup and Superhuman config"
read_when:
...
```bash
superhuman plugins install @openclaw/mattermost
```

Local checkout (when running from a git repo):

```bash
superhuman plugins install ./path/to/local/mattermost-plugin
```
...
4. Configure Superhuman and start the gateway.
...
Native slash commands are opt-in. When enabled, Superhuman registers `oc_*` slash commands via
...
- If `callbackUrl` is omitted, Superhuman derives one from gateway host/port + `callbackPath`.
...
- Reachability requirement: the callback endpoint must be reachable from the Mattermost server.
  - Do not set `callbackUrl` to `localhost` unless Mattermost runs on the same host/network namespace as Superhuman.
  - Do not set `callbackUrl` to your Mattermost base URL unless that URL reverse-proxies `/api/channels/mattermost/command` to Superhuman.
  - A quick check is `curl https://<gateway-host>/api/channels/mattermost/command`; a GET should return `405 Method Not Allowed` from Superhuman, not `404`.
...
- Approve via:
  - `superhuman pairing list mattermost`
  - `superhuman pairing approve mattermost <CODE>`
...
Use these target formats with `superhuman message send` or cron/webhooks:
...
Superhuman resolves them **user-first**:

- If the ID exists as a user (`GET /api/v4/users/<id>` succeeds), Superhuman sends a **DM** by resolving the direct channel via `/api/v4/channels/direct`.
...
When Superhuman sends to a Mattermost DM target and needs to resolve the direct channel first, it
...
- If `interactions.callbackBaseUrl` is omitted, Superhuman derives the callback URL from
...
- Reachability rule: the button callback URL must be reachable from the Mattermost server.
  `localhost` only works when Mattermost and Superhuman run on the same host/network namespace.
...
`superhuman message send` and cron/webhook deliveries.
